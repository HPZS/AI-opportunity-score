...
export function validateTaskResult(taskType: string, assistantText: string): { ok: boolean; message?: string } {
  if (taskType !== "screening" || !SCREENING_DEFAULT_CONFIG.enforceTargetPoolEntryCount) {
    return { ok: true };
  }
...
export async function runTaskWithSupervisor(
  ...
  let attemptCount = 1;
  let latestResult = await agent.runOnce(taskMessage);
  let validation = validateTaskResult(canonicalTaskType, latestResult.text);

  const shouldSupervise =
    canonicalTaskType === "screening" &&
    SCREENING_DEFAULT_CONFIG.enableTaskSupervisor &&
    SCREENING_DEFAULT_CONFIG.enforceTargetPoolEntryCount;

  while (
    shouldSupervise &&
    !validation.ok &&
    attemptCount < SCREENING_DEFAULT_CONFIG.maxSupervisorRuns
  ) {
    const nextAttempt = attemptCount + 1;
    printInfo(
      `初筛任务未达标，启动任务级自动续跑 (${nextAttempt}/${SCREENING_DEFAULT_CONFIG.maxSupervisorRuns})。`
    );
    const continuationPrompt = buildScreeningContinuationPrompt(latestResult.text, nextAttempt);
    latestResult = await agent.runOnce(continuationPrompt);
  ...
  if (
    ...
    printInfo(
      `已达到任务级自动续跑上限 (${SCREENING_DEFAULT_CONFIG.maxSupervisorRuns})，停止继续补跑。`
    );
  }
