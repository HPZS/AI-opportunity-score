...
export type OpportunityStage = "规划信号" | "招标中" | "中标后" | "合同签订" | "已落地" | "政策信号";
...
export const MATURITY_SIGNALS = {
  high: ["采购公告", "招标公告", "竞争性磋商", "中标", "立项", "项目预算", "实施方案", "采购需求", "采购文件", "服务采购"],
  medium: ["试点", "建设方案", "专项行动", "能力提升", "平台建设", "升级改造", "运行维护", "升级维护", "知识库维护", "热线平台升级", "智能客服", "智能质检", "工单分拨", "工单分派"],
  policy: ["通知", "意见", "专项资金", "政策支持", "行动计划", "工作要点"],
};

// 阶段识别规则：控制线索会被判成“招标中、合同签订、政策信号”等哪一种阶段。
export const STAGE_PATTERNS: Array<{ stage: OpportunityStage; patterns: RegExp[] }> = [
  {
    stage: "合同签订",
  ...
  {
    stage: "招标中",
    patterns: [/招标公告/, /采购公告/, /竞争性磋商/, /比选公告/, /招标文件/, /采购需求/, /采购文件/, /服务采购/, /运维服务/, /运行维护/, /公开招标/],
  },
  {
    stage: "已落地",
  ...
  {
    stage: "政策信号",
    patterns: [/工作要点/, /行动计划/, /实施意见/, /通知/, /工作情况/, /政策支持/, /指导意见/],
  },
  {
    stage: "规划信号",
    patterns: [/立项/, /可行性研究/, /可研/, /建设方案/, /升级改造/, /采购意向/, /需求征集/, /项目建议书/, /预算公开/],
  },
];
...
export const KEYWORD_SUBSCRIPTIONS: KeywordSubscription[] = [
  ...
  },
  {
    ...
    keywords: ["公文", "发文", "收文", "材料撰写", "智能校对", "知识库", "大模型"],
  ...
  },
  {
    ...
    keywords: ["知识库", "问答", "咨询导办", "智能体", "智能问答", "RAG", "语义检索"],
...
// 通过 id 获取单个信号源预设。
export function getSignalSourceProfile(id: string): SearchSourceProfile | undefined {
  return SIGNAL_SOURCE_PROFILES.find((item) => item.id === id);
}

// 通过 id 获取单个关键词订阅。
export function getKeywordSubscription(id: string): KeywordSubscription | undefined {
...
export function mergeSourceProfiles(profileIds: string[]): {
  ...
  return {
    ...
    // 搜索作用域会直接参与 query 拼装。
    searchScopes: unique(profiles.flatMap((item) => item.searchScopes)).slice(0, 6),
