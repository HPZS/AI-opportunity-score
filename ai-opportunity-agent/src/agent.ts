...
export class Agent {
  ...
  private configuredOpenAIBase?: string;
  private configuredAnthropicBase?: string;
...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that.`;
  ...
  private async executeAgentTool(input: Record<string, any>): Promise<string> {
    const type = (input.type || "analysis") as SubAgentType;
  ...
  private async chatOpenAI(userMessage: string): Promise<void> {
    ...
    while (true) {
      ...
      if (!choice) {
        throw new Error("OpenAI 兼容接口未返回 choices[0]，无法继续执行任务。");
      }
      ...
      if (!toolCalls || toolCalls.length === 0) {
        if (!assistantText) {
          throw new Error(
            `OpenAI 兼容接口返回了空响应：model=${this.model}，finish_reason=${choice.finish_reason || "unknown"}。`
            + " 当前响应没有 content，也没有 tool_calls。请优先检查上游模型是否真正支持该接口形态。"
          );
        }
        if (!this.isSubAgent) {
          printCost(this.totalInputTokens, this.totalOutputTokens);
  ...
  private async callOpenAIStream(): Promise<OpenAI.ChatCompletion> {
    return withRetry(async (signal) => {
      ...
      for (let emptyAttempt = 0; emptyAttempt <= maxEmptyRetries; emptyAttempt++) {
        ...
        if (!isEmptyOpenAIMessage(completion.choices[0]?.message)) {
          return completion;
        }
        ...
        printInfo(`OpenAI 兼容接口返回空响应，正在重试 (${emptyAttempt + 1}/${maxEmptyRetries})...`);
        await new Promise((resolve) => setTimeout(resolve, 1200 * (emptyAttempt + 1)));
      }

      throw new Error("OpenAI 兼容接口重试后仍未返回有效结果。");
    }, this.abortController?.signal);
  }
